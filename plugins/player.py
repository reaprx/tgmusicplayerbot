...
async def yplay(_, message: Message):
    if ADMIN_ONLY == "Y":
        ...
        if message.from_user.id not in admins:
            m=await message.reply_sticker("CI")
            await mp.delete(m)
...
async def deezer(_, message):
    if ADMIN_ONLY == "Y":
        admins = await mp.get_admins(CHAT)
        if message.from_user.id not in admins:
            k=await message.reply_sticker("CI")
            await mp.delete(k)
            await mp.delete(message)
            return
    user=f"[{message.from_user.first_name}](tg://user?id={message.from_user.id})"
...
async def not_chat(_, m: Message):
    buttons = [
        [
            InlineKeyboardButton('🧩 Source Code', url='https://github.com/reaprx/tgusicPlayer'),
        ],
        [
            InlineKeyboardButton('👨🏼‍🦯 Help', callback_data='help')       
        ]
        ]
    k=await m.reply("<b>You can't use this bot in this group, for that you have to make your own bot from the [SOURCE CODE](https://github.com/reaprx/tgusicPlayer) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
    await mp.delete(m)
